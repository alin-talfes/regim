...
begin
  ...
  if tg_op = 'INSERT' and new.data_depunerii <= local_today - 30 then
    raise exception 'Nu poti adauga o persoana aflata deja in Ziua 31 sau ulterior';
  end if;
...
comment on function private.touch_ppl() is
  'Validates PPL writes. Inserts are rejected when the deposit date already corresponds to quarantine day 31 or later. Day 31+ archival is logical in the application; rows remain stored in public.ppl.';
